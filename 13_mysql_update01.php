...
if(count($_POST)>0) {
    ...
    switch($_POST["wasTun"]) {
        ...
        case "aktualisieren":
            $id = $_POST["welcheID"]; //Hilfsvariable, eine "Abkürzung"
            
            if($_POST["GebDatum_" . $id]=="") {
                $geb = "NULL";
            }
            else {
                $geb = "'" . $_POST["GebDatum_" . $id] . "'";
            }
            ...
            $sql = "
                UPDATE tbl_user SET
                    ...
                    Emailadresse='" . $_POST["Emailadresse_" . $id] . "',
                    Passwort='" . $_POST["Passwort_" . $id] . "',
                    GebDatum=" . $geb . "
                WHERE(
                    IDUser=" . $id . "
                )
            ...
            $ok = $conn->query($sql) or die("Fehler in der Query: " . $conn->error);
            if($ok) {
                $msg = '<p class="success">Der Datensatz wurde erfolgreich geändert.</p>';
            }
            else {
                $msg = '<p class="error">Leider konnte der Datensatz nicht wie gewünscht geändert werden.</p>';
            }
           break;
    }
}
...
<title>MySQL</title>
    ...
    <script>
        ...
        function aktualisieren(zuAktualisierendeID) {
            document.getElementById("wasTun").value = "aktualisieren";
            document.getElementById("welcheID").value = zuAktualisierendeID;
            document.getElementById("frm").submit();
        }
    </script>
</head>
            ...
            <tbody>
                ...
                while($zeile = $antwort->fetch_object()) {
                    echo('
                        <tr>
                            <td>' . $zeile->IDUser . '</td>
                            <td><input type="text" value="' . $zeile->Vorname . '" name="Vorname_' . $zeile->IDUser . '"></td>
                            <td><input type="text" value="' . $zeile->Nachname . '" name="Nachname_' . $zeile->IDUser . '"></td>
                            <td><input type="email" value="' . $zeile->Emailadresse . '" name="Emailadresse_' . $zeile->IDUser . '"></td>
                            <td><input type="text" value="' . $zeile->Passwort . '" name="Passwort_' . $zeile->IDUser . '"></td>
                            <td><input type="date" value="' . $zeile->GebDatum . '" name="GebDatum_' . $zeile->IDUser . '"></td>
                            <td>' . $zeile->RegZeitpunkt . '</td>
                            <td>
                                <button type="button" onclick="loeschen(' . $zeile->IDUser . ');">DEL</button>
                                <button type="button" onclick="aktualisieren(' . $zeile->IDUser . ');">UPD</button>
                            </td>
                        </tr>
                    ');
                }
